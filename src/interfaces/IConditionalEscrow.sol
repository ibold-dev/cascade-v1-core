...
interface IConditionalEscrow {
    ...
    event Deposited(address indexed payer, address indexed payee, uint256 escrowId, uint256 amount);

    /// @notice Emitted when funds are released from escrow
    event Released(uint256 escrowId, address indexed payee, uint256 amount);

    /// @notice Emitted when funds are refunded from escrow
    event Refunded(uint256 escrowId, address indexed payer, uint256 amount);
